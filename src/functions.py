...
class XUIAPI:
    ...
    async def create_vless_profile(self, telegram_id: int, device_name: str = None):
        ...
        try:
            ...
            if await self.update_inbound(config.INBOUND_ID, update_data):
                return {
                    ...
                    "vless_url": self._generate_vless_url(client_id, email, inbound["port"])
                }
            return None
        except Exception as e:
    ...
    def _generate_vless_url(self, client_id: str, email: str, port: int) -> str:
        """Генерирует VLESS URL с параметрами Reality"""
        # Формируем fragment с именем пользователя
        username = f"vad1mus-VPN-{email}"
    ...
    async def delete_client(self, email: str):
        ...
        try:
            ...
            # Добавляем allocate если он есть
            if "allocate" in inbound:
                update_data["allocate"] = inbound["allocate"]
...
async def create_vless_profile(telegram_id: int, device_name: str = None):
    api = XUIAPI()
    try:
        return await api.create_vless_profile(telegram_id, device_name)
    finally:
        await api.close()
...
async def delete_vless_profile(email: str):
    """Удаляет VLESS профиль по email"""
    return await delete_client_by_email(email)
